...
export default function SignInModal({ open, onOpenChange }: SignInModalProps) {
  ...
  useEffect(() => {
    if (isSignedIn && open && !hasRedirectedRef.current && !isRedirectingRef.current) {
      debug.info({
        location: "SignInModal.tsx:useEffect",
      ...
      // Simplified redirect: close modal, wait, then redirect
      setTimeout(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    debug.info({
      ...
      data: { hasSignIn: !!signIn, emailLength: email.length },
    });
    ...
    try {
      ...
      if (result.status === "complete") {
          ...
          if (typeof window !== "undefined" && window.location) {
            ...
            setTimeout(() => {
              if (window.location.pathname !== "/app") {
                console.log('[DEBUG] Redirect failed, trying window.location.href = "/app"');
                window.location.href = "/app";
              }
            }, 100);
          } else {
            console.log('[DEBUG] Window not available, using router.push("/app") as fallback');
            router.push("/app");
          }
        } catch (redirectError) {
        ...
        debug.info({
          location: "SignInModal.tsx:handleSubmit",
          message: "Redirect command executed",
          data: {
            currentUrl: typeof window !== "undefined" ? window.location.href : "server",
          },
        });
        ...
        setError("התחברות לא הושלמה. סטטוס: " + result.status);
      }
    } catch (err: unknown) {
      debug.error({
        location: "SignInModal.tsx:handleSubmit",
      ...
      setError(error.errors?.[0]?.message || "התחברות נכשלה");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-gradient-to-br from-gray-900 via-gray-800 to-black border-gray-700 p-0">
